//SharedShaders/0x149de01e.ios_vcode uklgonzale Wed Mar 12 17:08:03 2014
 ...
 void main()
{
    ...
    _TMP21.xy = vec2(float((vec2(float(TEXCOORD1.x), float(TEXCOORD1.y)) + _lightmapOffset.xy).x), float((vec2(float(TEXCOORD1.x), float(TEXCOORD1.y)) + _lightmapOffset.xy).y));
